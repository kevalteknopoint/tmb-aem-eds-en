...
async function renderAllCards(block, allRows) {
  ...
  for (const row of allRows) {
    ...
    if (row.classList.contains('compare-accounts-wrapper')) {
      ...
      if (existingCard) {
        cardData = {
          ...
          title: existingCard.querySelector('.product-title')?.textContent.trim() || '',
          description: existingCard.querySelector('.product-description')?.textContent.trim() || '',
          pretitle: existingCard.querySelector('.product-interest-pretitle')?.textContent.trim() || '',
          rawRate: existingCard.querySelector('.product-interest-rate')?.textContent.trim() || '',
          disclaimer: existingCard.querySelector('.product-disclaimer')?.textContent.trim() || '',
          btnText: existingCard.querySelector('.button')?.textContent.trim() || 'Apply Now',
          btnLink: existingCard.querySelector('.button')?.getAttribute('href') || '#',
          btnTitle: existingCard.querySelector('.button')?.getAttribute('title') || '',
          btnTarget: existingCard.querySelector('.button')?.getAttribute('target') || '_self',
          btnStyle: [...existingCard.querySelector('.button')?.classList || []].find((c) => c !== 'button') || 'outlined',
        };
      }
    } else {
    ...
    if (cardData.title || cardData.description) {
      const card = div(
        { class: ['compare-accounts-card', cardData.isActive && 'active'].filter(Boolean) },
  ...
  const innerBlock = div({ class: 'compare-accounts block', 'data-block-name': 'compare-accounts' }, cardsContainer);
...
export default async function decorate(block) {
  ...
  for (const link of links) {
    ...
    if (fragment) {
      ...
      if (fragmentBlock) {
        const wrapper = fragmentBlock.querySelector('.compare-accounts-wrapper');
        if (wrapper) {
