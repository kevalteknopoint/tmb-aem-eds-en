...
document.addEventListener('click', (e) => {
  if (e.target.closest('.bannervideo-wrapper .button')) {
    ...
    const bannerName = e.target.closest('.bannervideo-wrapper').querySelector("h1,h2,h3,h4,h5,h6")?.getAttribute('id')
    ...
    bannerInteraction(pageRegion, minifyText(secondaryLink?.textContent), bannerName, carouselPosition, 'bannervideo', 'bannervideo', componentIndex, getPersona(), nextPageURL, 'banner-click', 'internal', '', '', '', 'homepage-banner', '', '');
  }
  if (e.target.closest('.bannervideo-wrapper .swiper-pagination-bullet')) {
    ...
    bannerInteraction('', minifyText(secondaryLink?.textContent), bannerName, carouselPosition, '', '', '', '', '', '', 'banner-click', 'internal', '', '', '', '', '');
  }
});
