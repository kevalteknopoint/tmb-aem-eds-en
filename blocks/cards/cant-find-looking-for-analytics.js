...
if ( e.target.closest('.navigation-cards-container .navigation-cards-wrapper:not(.rates-saver.faq-cant-find-looking-for-variant)') &&!e.target.closest('.navigation-cards-container .nav-card a.is-clickable')) {
  const link = e.target.closest('.navigation-cards-container a');
  if (!link) return;
  const container = link.closest('.navigation-cards-container');
  // ✅ SECTION HEADING (single source of truth)
  ...
  // ✅ REQUIRED FIELDS
  const ctaText = minifyText(link.textContent);   // "View details"
