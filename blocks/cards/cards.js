...
export default function decorate(block) {
  if (block.classList.contains('expandable-tiles')) {
    ...
    block.querySelectorAll('.online-banking .cards-wrapper .cards ul li:nth-child(2) .cards-card-body ul').forEach((blockUl, idx) => {
    ...
    block.querySelectorAll('.online-banking .cards-wrapper .cards ul li:nth-child(3) .cards-card-body ul').forEach((blockUl, idx) => {
      blockUl.classList.add(`card-bottom-${idx + 1}`);
    });
